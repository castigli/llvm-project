...
// CHECK-SAME:                            %[[IN:.*]]: tensor<10x10xf32>) -> tensor<10x10xf32> {
// CHECK-NOT: linalg.transpose
// CHECK: %{{.*}} = tensor.extract_slice %[[IN]][0, %[[INDEX:.*]]] [10, 1] [1, 1] : tensor<10x10xf32> to tensor<10x1xf32> 
// CHECK: %{{.*}} = tensor.insert_slice %{{.*}} into %{{.*}}[0, %[[INDEX]]] [10, 1] [1, 1] : tensor<10x1xf32> into tensor<10x10xf32>
// CHECK: %[[TRANSPOSED_0:.*]] = linalg.transpose ins(%{{.*}} : tensor<10x10xf32>) outs(%{{.*}} : tensor<10x10xf32>) permutation = [1, 0]
// CHECK: %[[TRANSPOSED_1:.*]] = linalg.transpose ins(%[[TRANSPOSED_0]] : tensor<10x10xf32>) outs(%{{.*}} : tensor<10x10xf32>) permutation = [1, 0]

func.func @transpose_scan(%arg0: tensor<10x10xf32>) -> tensor<10x10xf32> {
